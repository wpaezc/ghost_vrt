...
const config = require("./resemblejs_playwright_config.json");
const fs = require('fs');
...
async function executeTest(){
  ...
  for(scenario_index in playwright_scenarios) {
    ...
    if (!fs.existsSync(`./resemblejs_reports_playwright/${scenario}`)){
      fs.mkdirSync(`./resemblejs_reports_playwright/${scenario}`, { recursive: true });
    }
    ...
    for(pair in pairs) {
      ...
      fs.copyFileSync(`./playwright_screenshots/${scenario}/${pairs[pair][0]}`, `./resemblejs_reports_playwright/${scenario}/${pairs[pair][0]}`)
      fs.copyFileSync(`./playwright_screenshots/${scenario}/${pairs[pair][1]}`, `./resemblejs_reports-playwright/${scenario}/${pairs[pair][1]}`)
      fs.copyFileSync(`./index.css`, `./resemblejs_reports-playwright/${scenario}/index.css`)
      fs.writeFileSync(`./resemblejs_reports_playwright/${scenario}/${compare_name}`, data.getBuffer());
    ...
    fs.writeFileSync(
      `./resemblejs_reportspw/${scenario}/report.html`,
